...
module "three-tier-app-lb" {
    source = "./modules/terraform-google-solution-builder-external-application-load-balancer"
    project_id = var.project_id
    load_balancer_name = var.three-tier-app-lb-load_balancer_name
    load_balancer_port_name = var.three-tier-app-lb-load_balancer_port_name
    managed_instance_group_urls = var.three-tier-app-lb-managed_instance_group_urls
    managed_instance_group_health_check_links = var.three-tier-app-lb-managed_instance_group_health_check_links
    cloud_run_service_names = [module.three-tier-app-frontend.cloud_run_service_name]
    cloud_run_service_regions = [module.three-tier-app-frontend.cloud_run_service_region]
}
